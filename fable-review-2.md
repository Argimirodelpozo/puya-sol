...
| D2 🟡 | **`ReturnRewriter` redesign** — CHARACTERIZATION LANDED (2026-07-09): the WIRE-RETURN-TYPE spec table is now written at the top of the file, with an oracle fixture (tests/WIP/return-wire-oracle/: 33-method return-shape matrix + dumped wire types) — regenerate+diff gates any change. The table surfaced **2 probable live residuals**: (a) tuple with a DYNAMIC element leaves biguint elements unwrapped (uint512 vs caller's uintN — same class as the fixed (bytes4,uint128)); (b) MODIFIER'D biguint returns stay native/uint512 while cross-contract callers name declared width (crosscall fuzzer doesn't generate modifier'd callees). The 6-pass→plan-once rewrite itself: fresh-session job against the oracle | ReturnRewriter.cpp | High |
