...
def main() -> None:
    ...
    for name in ("initialize", "unlock", "modifyLiquidity", "settle"):
        s = sel[name]
    ...
    # 2) measure owed: simulate [unlock, modifyLiquidity], read modliq's DEBIT
    #    scratch slot (6) — the conflated single-currency amount the add-liq owes.
    from algosdk.v2client import models as _m
    mod_params = [60, 120, 1_000_000, bytes(32)]  # positive ticks, +liquidity

    def add_modliq(g, n_opup):
        for k in range(n_opup):
            g.add_app_call(au.AppCallParams(sender=sender, app_id=opup_id, note=f"o{k}".encode(),
        ...
        g.add_app_call_method_call(main_client.params.call(au.AppClientMethodCallParams(
            method="modifyLiquidity", args=[prep("modifyLiquidity"), pool_key, mod_params, b""],
    ...
    print("=== measuring owed (simulate [unlock, modifyLiquidity]) ===")
    gm = algorand.new_group(); add_modliq(gm, 8); gm.build()
    req = _m.SimulateRequest(txn_groups=[], allow_unnamed_resources=True, extra_opcode_budget=120000,
        exec_trace_config=_m.SimulateTraceConfig(enable=True, scratch_change=True))
    grp = gm._atc.simulate(algorand.client.algod, req).simulate_response["txn-groups"][0]
    ...
    for u in tr:
        for sc in u.get("scratch-changes", []):
            if sc.get("slot") == 6:
                nv = sc.get("new-value", {})
                owed = nv.get("uint", 0) if not nv.get("bytes") else int.from_bytes(base64.b64decode(nv["bytes"]), "big")
    print(f"  modliq owes (DEBIT slot 6, conflated single-currency) = {owed} microAlgos")
    assert owed > 0, "expected modliq to owe a positive amount"

    # 3) settle: [..unlock.., ..modifyLiquidity.., payment(owed)->pool, prepare, settle]
    #    settle reads the native payment grouped at GroupIndex-2 (prepare sits at -1),
    #    credits it, and _settleIfLast nets it against modliq's debit -> net-zero.
    print(f"=== settle group: [unlock, modifyLiquidity, payment({owed}), settle] ===")
    gs = algorand.new_group(); add_modliq(gs, 7)
    gs.add_payment(au.PaymentParams(sender=sender, receiver=main_client.app_address,
        amount=au.AlgoAmount.from_micro_algo(owed)))
    gs.add_app_call_method_call(main_client.params.call(au.AppClientMethodCallParams(
        method="settle", args=[prep("settle")], box_references=empties(4),
        static_fee=au.AlgoAmount.from_micro_algo(3000))))
    r = gs.send({"populate_app_call_resources": True, "cover_app_call_inner_transaction_fees": True})
    print("  settle returns:", [x.value for x in (r.returns or []) if x is not None])
    print("  pool boxes after settle:",
          len(algorand.client.algod.application_boxes(main_id).get("boxes", [])))
    print("\n\u2705 PASS: V4 modifyLiquidity completed E2E on the AVM \u2014 full liquidity "
          "math + delta accounting, native payment settled the owed currency, atomic-group "
          "net-zero check passed.")
