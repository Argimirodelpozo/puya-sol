...
import {bytesToG1Point, bytesToFr, bytesToG1PointMem, bytesToFrMem, _wordMem} from "./utils.sol";
...
library TranscriptLib {
    ...
    function loadProof(bytes memory proof, uint256 logN) internal pure returns (Honk.Proof memory p) {
        uint256 boundary = 0x00;

        // Pairing point object
        for (uint256 i = 0; i < PAIRING_POINTS_SIZE; i++) {
            uint256 limb = _wordMem(proof, boundary);
        ...
        // Commitments
        p.w1 = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;
        p.w2 = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;
        p.w3 = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;

        // Lookup / Permutation Helper Commitments
        p.lookupReadCounts = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;
        p.lookupReadTags = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;
        p.w4 = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;
        p.lookupInverses = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;
        p.zPerm = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;

        // Sumcheck univariates
        for (uint256 i = 0; i < logN; i++) {
            for (uint256 j = 0; j < BATCHED_RELATION_PARTIAL_LENGTH; j++) {
                p.sumcheckUnivariates[i][j] = bytesToFrMem(proof, boundary);
                boundary += FIELD_ELEMENT_SIZE;
            }
        }
        // Sumcheck evaluations
        for (uint256 i = 0; i < NUMBER_OF_ENTITIES; i++) {
            p.sumcheckEvaluations[i] = bytesToFrMem(proof, boundary);
            boundary += FIELD_ELEMENT_SIZE;
        }
        ...
        for (uint256 i = 0; i < logN - 1; i++) {
            p.geminiFoldComms[i] = bytesToG1PointMem(proof, boundary);
            boundary += GROUP_ELEMENT_SIZE;
        }

        // Read gemini a evaluations
        for (uint256 i = 0; i < logN; i++) {
            p.geminiAEvaluations[i] = bytesToFrMem(proof, boundary);
            boundary += FIELD_ELEMENT_SIZE;
        }

        // Shplonk
        p.shplonkQ = bytesToG1PointMem(proof, boundary);
        boundary += GROUP_ELEMENT_SIZE;
        // KZG
        p.kzgQuotient = bytesToG1PointMem(proof, boundary);
    }
}
