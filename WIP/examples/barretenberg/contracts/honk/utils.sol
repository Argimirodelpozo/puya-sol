...
// AVM proof-from-box reads. The 3904-byte proof exceeds the 2048-byte
// ApplicationArgs cap, so it cannot be a calldata arg — it is box-loaded into
// `bytes memory`. The vendored solc forbids slicing `bytes memory`, so the
// decode path reads words by byte-offset out of the in-memory buffer instead.
function _wordMem(bytes memory d, uint256 off) pure returns (uint256 w) {
    for (uint256 i = 0; i < 32; i++) {
        w = (w << 8) | uint256(uint8(d[off + i]));
    }
}

function bytesToFrMem(bytes memory d, uint256 off) pure returns (Fr scalar) {
    scalar = FrLib.from(_wordMem(d, off));
}

function bytesToG1PointMem(bytes memory d, uint256 off) pure returns (Honk.G1Point memory point) {
    uint256 x = _wordMem(d, off);
    uint256 y = _wordMem(d, off + 0x20);
    require(x < Q && y < Q, Errors.ValueGeGroupOrder());
    point = Honk.G1Point({x: x, y: y});
}
 ...
 */
function ecMul(Fr value, Honk.G1Point memory point) view returns (Honk.G1Point memory result) {
    // AVM port: the upstream version marshalled operands via inline-assembly EVM
    // memory pointers (mload(point)/mstore(free,...)), which has no AVM analogue.
    // Replaced with the abi.encodePacked + precompile-staticcall idiom pairing()
    // uses below; puya-sol routes 0x07 → AVM `ec_scalar_mul BN254g1`. The elliptic
    // curve arithmetic is identical.
    bytes memory input = abi.encodePacked(point.x, point.y, Fr.unwrap(value));
    (bool success, bytes memory ret) = address(0x07).staticcall(input);
    require(success);
    (result.x, result.y) = abi.decode(ret, (uint256, uint256));
}
 ...
 */
function ecAdd(Honk.G1Point memory lhs, Honk.G1Point memory rhs) view returns (Honk.G1Point memory result) {
    // AVM port: abi.encodePacked + 0x06 precompile (puya-sol → AVM `ec_add
    // BN254g1`), replacing the upstream inline-assembly memory-pointer marshalling.
    bytes memory input = abi.encodePacked(lhs.x, lhs.y, rhs.x, rhs.y);
    (bool success, bytes memory ret) = address(0x06).staticcall(input);
    require(success);
    (result.x, result.y) = abi.decode(ret, (uint256, uint256));
}
