...
def deploy_contract(
    ...
    _deploy_counter += 1
    app_spec = load_arc56(name, subdir=subdir)
    if app_args or extra_pages:
        ...
        # Use unique note to avoid duplicate txn IDs across test modules
        client, _txn = factory.send.bare.create(
            au.AppFactoryCreateParams(
                note=f"deploy-{name}-{_deploy_counter}".encode(),
            )
        )
    if fund_amount > 0:
...
def deploy_contract_raw(
    ...
    algod = localnet.client.algod
    base_dir = OUT_DIR / (subdir or name)
    approval_path = base_dir / f"{name}.approval.teal"
    clear_path = base_dir / f"{name}.clear.teal"
...
def box_key(mapping_name: str, *keys: bytes) -> bytes:
    """Compute box key for a mapping: sha256(concat(keys))."""
    concat = b"".join(keys)
    return mapping_name.encode() + hashlib.sha256(concat).digest()
