...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d9_7).
contract Kd9_7_0 {
    uint256 public log;
...
contract Kd9_7_1 is Kd9_7_0 {
    function f0(uint256 a) public override { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
    function f1(uint256 a) public override virtual mTwice() mArg(a % 5) { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
    function f2(uint256 a) public override virtual mBoth() mTwice() returns (uint256) { unchecked { log = log*100 + 22 + (a % 7); } return super.f2(a); }
}
contract Kd9_7_2 is Kd9_7_1 {
    function f1(uint256 a) public override mPre() { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; super.f1(a); }
    function f2(uint256 a) public override mBoth() mBoth() returns (uint256) { unchecked { log = log*100 + 22 + (a % 7); } return log; }
}