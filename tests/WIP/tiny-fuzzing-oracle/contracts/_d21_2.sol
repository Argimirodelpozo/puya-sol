...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d21_2).
contract Kd21_2_0 {
    uint256 public log;
    ...
    function f1(uint256 a) public mBoth() mPre() mPost() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public virtual mBoth() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd21_2_1 is Kd21_2_0 {
    function f0(uint256 a) public override { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
    function f2(uint256 a) public override mGate() mPost() mArg(a % 5) { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
}