...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d15_2).
contract Kd15_2_0 {
    uint256 public log;
    ...
    function f1(uint256 a) public virtual mGate() mTwice() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public mGate() mPre() mBoth() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public virtual mPre() mPre() returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return log; }
}
contract Kd15_2_1 is Kd15_2_0 {
    function f0(uint256 a) public override mPost() { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; }
