...
// GENERATED nested-aggregate fixture (fuzz_nested.py, tag nst47000_4, container map).
contract G_nst47000_4 {
    struct L0 { int8 a0; bytes32 a1; }
    struct L1 { L0 inner; int64[] xs; bool flag; }
    struct L2 { uint64 tag; L1 mid; }
    mapping(uint256 => L2) m;
    function set_a0(uint256 k, int8 v) external { m[k].mid.inner.a0 = v; }
    function get_a0(uint256 k) external view returns (int8) { return m[k].mid.inner.a0; }
    function bump_a0(uint256 k, int8 d) external { unchecked { m[k].mid.inner.a0 += d; } }
    function set_a1(uint256 k, bytes32 v) external { m[k].mid.inner.a1 = v; }
    function get_a1(uint256 k) external view returns (bytes32) { return m[k].mid.inner.a1; }
    ...
    function push_xs(uint256 k, int64 v) external { m[k].mid.xs.push(v); }
    function get_xs(uint256 k, uint256 i) external view returns (int64) { if (m[k].mid.xs.length <= i) return int64(0); return m[k].mid.xs[i]; }
