...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d5_1).
contract Kd5_1_0 {
    uint256 public log;
    ...
    function f1(uint256 a) public virtual mTwice() mBoth() { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
    function f2(uint256 a) public virtual mPre() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd5_1_1 is Kd5_1_0 {
    function f0(uint256 a) public override virtual mGate() mGate() mBoth() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return super.f0(a); }
    function f1(uint256 a) public override virtual mGate() mTwice() mTwice() { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
    function f2(uint256 a) public override mGate() mGate() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
}
contract Kd5_1_2 is Kd5_1_1 {
    function f0(uint256 a) public override virtual mBoth() mArg(bump(a)) mPre() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return super.f0(a); }
    function f1(uint256 a) public override virtual { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; super.f1(a); }
}
contract Kd5_1_3 is Kd5_1_2 {
