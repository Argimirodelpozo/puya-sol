...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d7_5).
contract Kd7_5_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public virtual mTwice() mTwice() mPre() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public virtual mPost() mPost() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public virtual mGate() mArg(bump(a)) mBoth() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd7_5_1 is Kd7_5_0 {
    function f0(uint256 a) public override mBoth() mPost() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
