...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d22_2).
contract Kd22_2_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public virtual mPost() mArg(bump(a)) mBoth() { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; }
    function f1(uint256 a) public virtual returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public mGate() mPost() mArg(bump(a)) { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public virtual mGate() mPre() returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return log; }
}
contract Kd22_2_1 is Kd22_2_0 {
    function f1(uint256 a) public override virtual returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return super.f1(a); }
    function f3(uint256 a) public override mBoth() mTwice() returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return super.f3(a); }
}
contract Kd22_2_2 is Kd22_2_1 {
    function f0(uint256 a) public override { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
}
contract Kd22_2_3 is Kd22_2_2 {
    function f1(uint256 a) public override mArg(a % 5) returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return super.f1(a); }
}