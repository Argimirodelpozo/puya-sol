...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d12_7).
contract Kd12_7_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public mPost() mArg(bump(a)) mPost() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public virtual mBoth() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
}
contract Kd12_7_1 is Kd12_7_0 {
    function f1(uint256 a) public override mPre() mTwice() mArg(a % 5) returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
}