...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d1_0).
contract Kd1_0_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public mBoth() { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; }
    ...
    function f3(uint256 a) public virtual mArg(a % 5) mBoth() { unchecked { log = log*100 + 23 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd1_0_1 is Kd1_0_0 {
    function f3(uint256 a) public override mTwice() mPre() { unchecked { log = log*100 + 23 + (a % 7); } sum += (a % 13) + 1; super.f3(a); }
}
contract Kd1_0_2 is Kd1_0_1 {
    function f1(uint256 a) public override returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public override mGate() mTwice() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
}