...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d23_4).
contract Kd23_4_0 {
    uint256 public log;
    ...
    function f1(uint256 a) public virtual mPost() mArg(a % 5) mArg(a % 5) { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
    function f2(uint256 a) public virtual { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public mTwice() mPost() returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return log; }
}
contract Kd23_4_1 is Kd23_4_0 {
    function f0(uint256 a) public override mGate() mBoth() mGate() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public override mBoth() mArg(a % 5) { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
