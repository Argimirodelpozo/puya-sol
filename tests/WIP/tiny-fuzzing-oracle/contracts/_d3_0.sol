...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d3_0).
contract Kd3_0_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public virtual mPre() mArg(bump(a)) mBoth() { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; }
    function f1(uint256 a) public mGate() mArg(a % 5) mGate() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public virtual mGate() mTwice() mGate() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd3_0_1 is Kd3_0_0 {
    function f0(uint256 a) public override mArg(a % 5) mBoth() { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
    function f2(uint256 a) public override mPost() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
}
contract Kd3_0_2 is Kd3_0_1 {
}