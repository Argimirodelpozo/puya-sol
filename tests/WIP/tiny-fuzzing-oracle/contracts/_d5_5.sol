...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d5_5).
contract Kd5_5_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public virtual mTwice() mArg(a % 5) returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public virtual returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
}
contract Kd5_5_1 is Kd5_5_0 {
    function f0(uint256 a) public override mTwice() mBoth() mBoth() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public override mGate() mGate() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return super.f1(a); }
}