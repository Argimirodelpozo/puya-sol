...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d15_4).
contract Kd15_4_0 {
    uint256 public log;
    ...
    function f2(uint256 a) public virtual mGate() mPre() mGate() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public virtual returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return log; }
}
contract Kd15_4_1 is Kd15_4_0 {
    function f0(uint256 a) public override { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
    function f1(uint256 a) public override virtual mTwice() mPre() mGate() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return super.f1(a); }
    function f2(uint256 a) public override virtual mPre() mPre() mGate() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public override returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return super.f3(a); }
}
contract Kd15_4_2 is Kd15_4_1 {
    function f1(uint256 a) public override mArg(a % 5) mArg(a % 5) returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public override mArg(bump(a)) mPost() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}