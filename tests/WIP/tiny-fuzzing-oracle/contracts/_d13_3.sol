...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d13_3).
contract Kd13_3_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public mTwice() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public virtual returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public virtual mPost() mPost() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd13_3_1 is Kd13_3_0 {
    function f1(uint256 a) public override returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return super.f1(a); }
    function f2(uint256 a) public override mPre() mTwice() mArg(bump(a)) { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}