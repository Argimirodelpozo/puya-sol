...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d17_4).
contract Kd17_4_0 {
    uint256 public log;
    ...
    function f1(uint256 a) public mTwice() mTwice() mGate() { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
    function f2(uint256 a) public mPost() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public mTwice() mPre() mTwice() { unchecked { log = log*100 + 23 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd17_4_1 is Kd17_4_0 {
    function f0(uint256 a) public override mBoth() mGate() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return super.f0(a); }
}