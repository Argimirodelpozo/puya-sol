...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d20_4).
contract Kd20_4_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public virtual mTwice() mPost() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public mBoth() mTwice() mTwice() { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
    function f2(uint256 a) public virtual returns (uint256) { unchecked { log = log*100 + 22 + (a % 7); } return log; }
}
contract Kd20_4_1 is Kd20_4_0 {
    function f0(uint256 a) public override mGate() mPost() mPost() returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f2(uint256 a) public override mBoth() mGate() returns (uint256) { unchecked { log = log*100 + 22 + (a % 7); } return log; }
}