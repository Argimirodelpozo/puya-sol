...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d3_4).
contract Kd3_4_0 {
    uint256 public log;
    ...
    function f1(uint256 a) public virtual mTwice() mTwice() mArg(a % 5) returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public virtual { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd3_4_1 is Kd3_4_0 {
    function f0(uint256 a) public override mArg(bump(a)) mPost() { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
    function f1(uint256 a) public override mPost() mPre() mArg(a % 5) returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return super.f1(a); }
    function f2(uint256 a) public override mPre() mGate() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
}