...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d18_4).
contract Kd18_4_0 {
    uint256 public log;
    ...
    function f1(uint256 a) public virtual mTwice() mGate() mArg(bump(a)) returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public virtual mPost() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public virtual mArg(bump(a)) mBoth() mBoth() { unchecked { log = log*100 + 23 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd18_4_1 is Kd18_4_0 {
    function f0(uint256 a) public override virtual { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
    function f2(uint256 a) public override virtual mGate() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
    function f3(uint256 a) public override mArg(bump(a)) { unchecked { log = log*100 + 23 + (a % 7); } sum += (a % 13) + 1; super.f3(a); }
}
contract Kd18_4_2 is Kd18_4_1 {
    ...
    function f2(uint256 a) public override mArg(a % 5) mGate() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
}