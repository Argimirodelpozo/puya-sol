...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d22_7).
contract Kd22_7_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; }
    function f1(uint256 a) public virtual mPost() { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; }
    function f2(uint256 a) public virtual mPre() mTwice() returns (uint256) { unchecked { log = log*100 + 22 + (a % 7); } return log; }
}
contract Kd22_7_1 is Kd22_7_0 {
    function f1(uint256 a) public override mPost() { unchecked { log = log*100 + 21 + (a % 7); } sum += (a % 13) + 1; super.f1(a); }
    function f2(uint256 a) public override mPost() returns (uint256) { unchecked { log = log*100 + 22 + (a % 7); } return log; }
}