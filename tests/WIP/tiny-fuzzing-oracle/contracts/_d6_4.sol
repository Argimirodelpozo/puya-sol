...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d6_4).
contract Kd6_4_0 {
    uint256 public log;
    ...
    function f2(uint256 a) public virtual mPre() mPost() mBoth() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public virtual mBoth() returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return log; }
}
contract Kd6_4_1 is Kd6_4_0 {
    function f0(uint256 a) public override mPost() mTwice() mTwice() { unchecked { log = log*100 + 20 + (a % 7); } sum += (a % 13) + 1; super.f0(a); }
    function f1(uint256 a) public override virtual mPre() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f3(uint256 a) public override returns (uint256) { unchecked { log = log*100 + 23 + (a % 7); } return super.f3(a); }
}
contract Kd6_4_2 is Kd6_4_1 {
    function f1(uint256 a) public override mTwice() returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return super.f1(a); }
    function f2(uint256 a) public override virtual mPost() mBoth() mArg(a % 5) { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
}
contract Kd6_4_3 is Kd6_4_2 {
    function f2(uint256 a) public override mTwice() mBoth() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; super.f2(a); }
}