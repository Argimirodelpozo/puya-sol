...
// GENERATED modifier/inheritance/dispatch fixture (fuzz_dispatch.py, tag d3_8).
contract Kd3_8_0 {
    uint256 public log;
    ...
    function f0(uint256 a) public mPost() mArg(bump(a)) returns (uint256) { unchecked { log = log*100 + 20 + (a % 7); } return log; }
    function f1(uint256 a) public returns (uint256) { unchecked { log = log*100 + 21 + (a % 7); } return log; }
    function f2(uint256 a) public mBoth() mPost() mBoth() { unchecked { log = log*100 + 22 + (a % 7); } sum += (a % 13) + 1; }
    function f3(uint256 a) public virtual { unchecked { log = log*100 + 23 + (a % 7); } sum += (a % 13) + 1; }
}
contract Kd3_8_1 is Kd3_8_0 {
    function f3(uint256 a) public override mGate() { unchecked { log = log*100 + 23 + (a % 7); } sum += (a % 13) + 1; super.f3(a); }
}