...
def test_event_selector(harness):
    ...
    # EVM_DIVERGENCE: event selectors are 32-byte sha512_256 on AVM.
    from framework import arc4_event_topic
    e0, eu = arc4_event_topic("E()"), arc4_event_topic("E(uint256)")
    assert tuple(as_int(x) for x in r.abi_return) == (e0, eu, e0, e0)
    ...
    assert as_int(r.abi_return) == arc4_event_topic("F()")
...
def test_event_selector_file_level(harness):
    ...
    # EVM_DIVERGENCE: sha512_256 event selectors.
    from framework import arc4_event_topic
    e0 = arc4_event_topic("E()")
    assert tuple(as_int(x) for x in r.abi_return) == (e0, e0)
...
def test_event_shadowing_file_level(harness):
    ...
    # EVM_DIVERGENCE: sha512_256 event selectors.
    from framework import arc4_event_topic
    es, e0 = arc4_event_topic("E(string)"), arc4_event_topic("E()")
    assert tuple(as_int(x) for x in r.abi_return) == (es, es, e0)
    ...
    assert tuple(as_int(x) for x in r.abi_return) == (e0, es, e0)
