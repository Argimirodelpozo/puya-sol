...
// Flattened OZ v5 TRANSPARENT proxy closure (verbatim bodies) + a plain
// implementation and a proxy factory. Guards proxy.md Â§2 lowering.
...
contract TranspFactory {
    function deployProxy(address impl, address admin) external returns (address) {
        return address(new TransparentUpgradeableProxy(impl, admin, ""));
    }
}
