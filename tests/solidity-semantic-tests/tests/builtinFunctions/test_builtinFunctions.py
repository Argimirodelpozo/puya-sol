"""Auto-generated tests for the builtinFunctions category.
...
def test_erc7201_param_locations(harness):
    ...
    r = harness.call(app, "calldataParam(string)", 32, 12, bytes.fromhex('6578616d706c652e6d61696e'))
    assert r.abi_return is True
    # calldataSlice(bytes): 0x20, 12, "example.main" -> true
    r = harness.call(app, "calldataSlice(bytes)", 32, 12, bytes.fromhex('6578616d706c652e6d61696e'))
    assert r.abi_return is True
    # literalParam() -> true
...
def test_function_types_sig(harness):
    ...
    r = harness.call(app, "g()")
    assert r.abi_return == 34249731031114569470249586604156090982925900387213360426261081278801594286080
...
def test_keccak256(harness):
    ...
    assert r.abi_return == 1546678032441257452667456735582814959992782782816731922691272282333561699760
...
def test_keccak256_packed(harness):
    ...
    r = harness.call(app, "f(int256)", 5)
    assert r.abi_return == 109899912411597832212822893111608843524663982160387972078328824211481649590333
...
def test_ripemd160(harness):
    ...
    r = harness.call(app, "f(int256)", 5)
    assert r.abi_return == 107800049998410314181947434501784187786654500865778811479236083403822943174656
...
def test_ripemd160_packed(harness):
    ...
    r = harness.call(app, "f(int256)", 5)
    assert r.abi_return == 2242101781399935236999115622957774579424822401483030762403486566236383346688
...
def test_sha256(harness):
    ...
    r = harness.call(app, "f(int256)", 5)
    assert r.abi_return == 68240159698054048912945530158929160304440341641269126469902820990734598294599
...
def test_sha256_packed(harness):
    ...
    r = harness.call(app, "f(int256)", 5)
    assert r.abi_return == 105518105313395515086322545934905310180852738469466176175217259920264724364528
    # f(int256): -1 -> 0xf14def4d07cd185ddd8b10a81b2238326196a38867e6e6adbcc956dc913488c7
    r = harness.call(app, "f(int256)", 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff)
    assert r.abi_return == 109145095326669468812651857352188286659179238190865846698410948873810387175623
