"""Tests for the constants category."""
import pytest
...
def test_constant_string(harness):
    ...
    r = harness.call(app, "f()")
    assert bytes(r.abi_return) == b"\x03\x01\x02"
...
def test_constant_string_at_file_level(harness):
    ...
    assert tuple(as_int(x) for x in r.abi_return) == (56, 1, 44048180624707321370159228589897778088919435935156254407473833945046349512704)
...
def test_consteval_array_length(harness):
    ...
    # f() -> 0x0a, 0x0a
    r = harness.call(app, "f()")
    assert tuple(as_int(x) for x in r.abi_return) == (10, 10)
...
def test_same_constants_different_files(harness):
    ...
    r = harness.call(app, "f()")
    assert tuple(as_int(x) for x in r.abi_return) == (13, 89, 89, 89)
