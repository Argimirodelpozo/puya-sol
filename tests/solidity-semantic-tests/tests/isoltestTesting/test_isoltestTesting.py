...
def test_precompiles_ignoring_trailing_input(harness):
    ...
    Each method returns `bytes memory` from the precompile call result.
    Verify the call succeeds and returns non-empty bytes."""
    app = harness.compile_and_deploy("isoltestTesting/contracts/precompiles_ignoring_trailing_input.sol")
    h = 0x18c547e4f7b0f325ad1e56f57e26c745b09a3e503d86e00e5255ff7f715d3d1c
    r1 = 0x73b1693892219d736caba55bdb67216e485557ea6b6af75f37096c9aa6a5a75f
    r2 = 0xeeb940b1d03b21e36b0e47e79769f095fe2ab855bd91e3a38756b7d75a9c4549
    assert len(bytes(harness.call(app, "ecRecover(uint256[4])", [h, 28, r1, r2]).abi_return)) > 0
    assert len(bytes(harness.call(app, "ecAdd(uint256[4])", [1, 2, 1, 2]).abi_return)) > 0
    assert len(bytes(harness.call(app, "ecMul(uint256[3])", [1, 2, 2]).abi_return)) > 0
