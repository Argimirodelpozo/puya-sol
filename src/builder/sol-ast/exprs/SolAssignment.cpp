...
std::shared_ptr<awst::Expression> SolAssignment::toAwst()
{
	...
	// Normalize the LHS to a writable form: unwrap any StateGet / ARC4Decode
	// (read-only wrappers) at the top of the chain or anywhere inside an
	// IndexExpression / FieldExpression base chain. Puya rejects these as
	// lvalues.
	target = awst::makeWritableTarget(target);
		...
		if (targetIsArc4)
		{
			...
			if (!sameShape)
			{
				...
				bool const targetIsArc4Array =
					target->wtype->kind() == awst::WTypeKind::ARC4StaticArray
					|| target->wtype->kind() == awst::WTypeKind::ARC4DynamicArray;
				if (sourceIsArc4Array && targetIsArc4Array)
				{
					...
					auto srcAsBytes = awst::makeReinterpretCast(
						value, awst::WType::bytesType(), m_loc);
