...
private:
	...
	/// Set/clear the fresh placeholder-call factory during modifier translation.
	void setPlaceholderFactory(sol_ast::PlaceholderFactory _factory);
	...
	std::vector<std::shared_ptr<awst::Statement>> makeParamDecodeStatements(
		std::vector<ParamDecode> const& _paramDecodes);
	/// Top-level whole rebinds of an aliased memory modifier param: statement
	/// → (param, AWST type). Consumed by buildModifierChain's statement hook.
	using ModifierRebindPlans = std::map<solidity::frontend::Statement const*,
		std::pair<solidity::frontend::VariableDeclaration const*, awst::WType const*>>;
	/// Bind one modifier invocation's arguments for the chain link being built:
	/// binding statements go to `_modBody`, every remapped declaration to
	/// `_remappedDeclIds`, and the alias-mode rebinds to `_rebindPlans`.
