...
- **Storage metadata is not a materialized value** — solc's full-width fixed-array lengths and logical slots remain available even for enormous declarations. Unsupported whole-value accesses produce capacity diagnostics; sparse element access requires explicit `--evm-storage-layout`. The compiler never switches storage formats automatically.
- **Raw array storage requires slot mode** — array `sload`/`sstore` and assembly-rebound array references require `--evm-storage-layout`. Named storage cannot preserve hidden elements across raw length shrink/regrow. With named arrays, unproven raw slots are rejected; proven scalar state slots, ordinary array operations and pure slot metadata remain supported.
- **Absent fixed boxes have Solidity defaults** — bounded reads of large fixed mapping values return defaults without creating boxes. Partial writes create valid backing storage; deleted multi-box arrays can be read and reused. Named-cell mapping values larger than one 32 KiB box and interior references to large fixed arrays are explicitly unsupported; these are capacity limits, not a reason to guess a value or key.
- **Mapping holders use solc coordinates** — default-layout roots containing mappings use versioned full-width solc slots; tagged member/array/mapping steps derive descendant hash keys. Flat array values remain indexed inside their containing box. Direct accesses, aliases, reference arguments and getters share the encoding. This is a fresh-deployment-only format break; existing applications must keep their original compiler/artifacts. See [storage format 2](docs/storage-format.md), including ARC-56 metadata and key-construction tooling.
- **Assembly memory spans scratch slots** — byte ranges and words can cross 4,096-byte slot boundaries. `--evm-memory-slots` controls the bounded region (default: five slots, 20 KiB). This is not a universal EVM memory model for all Solidity values. See `src/builder/target/ScratchLayout.h` and `src/builder/yul/MemoryHelpers.cpp`.
- **Inline assembly is supported but limited** — Yul memory/storage operations, arithmetic, [user functions](docs/yul-subroutines.md), and recognized precompile calls have explicit lowerings in `src/builder/yul/`. Supported storage behavior depends on the selected profile. EVM-only operations are subject to the same documented fidelity policy; accepting an assembly block does not establish arbitrary EVM equivalence.
...
- **Transient layout comes from solc** — typed scalar access shares the canonical word codec. Native address high bytes live in private scratch; a raw `tstore` clears them only for its logical word. See [the approved adaptation](EVM_DIVERGENCE.md#transient-words-and-native-addresses).
